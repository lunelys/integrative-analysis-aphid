...
for gender in "${genders[@]}"; do
    for reptype in "${reptypes[@]}"; do
        for peaktype in "${peaktypes[@]}"; do
            # Construct filenames
            deg_file="${RNA_directory}DEG_${gender}_tss.bed"
            dar_file="${FAIRE_directory}DAR_${peaktype}_${gender}_${reptype}.bed"
            output_file="${integrated_directory}DAR_${peaktype}_DEG_${gender}_${reptype}_overlap${overlap_fraction}.bed"

            # Run bedtools intersect with each combination
            bedtools intersect -F "$overlap_fraction" -a "$deg_file" -b "$dar_file" -wb > "$output_file"
